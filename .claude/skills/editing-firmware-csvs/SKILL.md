...
- **global_variables** — type width MUST match the load (`byte`=lbz/1B, `word`=lhz/2B, `dword`=lwz/4B). Leading `_name` = type too narrow → see underscore fix. **`_cal` = EEPROM-backed (Calterm-tunable) only — classify by the MEMORY MAP, not e2m** (e2m uses Calterm virtual addresses, so a RAM-address grep is blind): ROM / the ROM→RAM copy window (0x3F9800–0x3FDB30) / Bank2 flash = fixed constant → **no `_cal`**; EEPROM (0x01xxxxxx) or an EEPROM-loaded RAM value → `_cal`; computed/working RAM vars → never `_cal`.
